	...
	public function onRun($timer = null){
		...
		switch($this->stat){
			...
			if(count($players) >= $nec){
				...
				$this->broadcast("§l§bTHEBRIDGE\n§r§b§•Esperando Jugadores " . ($nec - count($players)). " ...");
			}
			break;
			...
			if(count($players) < $nec){
				...
				if($time <= 0){
					...
					$message = "§•§a§lEl JUEGO COMIENZA EN:\n§r§6§b§f $temp ";
			...
			if(!is_null($level)){
				foreach($level->getPlayers() as $p){
				    //A cool message
				$this->broadcast("§l§e✘Usted Esta En\n§r§b§lThebridge✘§8\n§8\n§8\n§8\n", 2);

//Scoreboard
$api = Scoreboards::getInstance();
$api->new($p, "TheBridge", T::AQUA . T::AQUA . "THEBRIDGE" . T::RESET . T::GREEN . " ");
$api->setLine($p, 1, T::GRAY." ".date("d/m/Y").T::BLACK." ");
$api->setLine($p, 2, T::RED."  ");
$api->setLine($p, 3, T::BLUE."Blue Team: ".T::YELLOW.$this->ponts["blue"].T::GRAY."/5");
$api->setLine($p, 4, T::RED."Red Team: ".T::YELLOW.$this->ponts["red"].T::GRAY."/5");
$api->setLine($p, 5, T::RED."    ");
$api->setLine($p, 6, T::RED."§rPlayers: ".T::YELLOW. count($this->players));
$api->setLine($p, 7, T::RED."§rNick: ".T::GREEN.$p->getName());
$api->setLine($p, 8, T::RED." ");
$api->setLine($p, 9, T::GREEN."§rModo: ".T::GREEN.$this->data["mode"]);
$api->setLine($p, 10, T::RED."§rMapa: §aTheBridge");
$api->getObjectiveName($p);
	...
	private function setRadomTeam($name){
		...
		if(!$data->isInTeam($name)){
			if($this->setTeam("blue", $name)){
				$this->broadcast("§a§l✘$name §b§lSe A Unido Al Equipo §1Blue✘", 3);
			} elseif($this->setTeam("red", $name)){
				$this->broadcast("§a§l✘$name §b§lSe A Unido Al Equipo §4Red✘", 3);
			}
			return true;
		}
		return false;
	...
	private function setTeams(){
		...
		foreach($players as $name => $p){
			if(!$data->isInTeam($name)){
				if($this->setTeam("blue", $name)){
					$this->broadcast("§a§l✘$name §bSe A Unido Al Equipo §1Blue✘", 3);
				} elseif($this->setTeam("red", $name)){
					$this->broadcast("§a§l✘$name §b§lSe A Unido Al Equipo §4Red✘", 3);
				}
			}
...
$api->setLine($p, 1, T::GRAY." ".date("d/m/Y").T::BLACK." ");
$api->setLine($p, 2, T::RED."  ");  
$api->setLine($p, 3, T::RED."§l§cTEAM: ".$team);                          
				continue;
			}
	...
	public function useHab($item, $p){
		...
		if($custom == "§r§l§e  HABILIDADES§r\n§7(Click para ver)"){
			...
			$inv->setItem(7, Item::get(262, 0, 1)->setCustomName("§l§c  Volver§r\n§7(Click para volver)§r"));
			return true;
		}
		if(isset($this->custons[$custom])){
			$perm = $this->custons[$custom];
			if(!$p->hasPermission($perm)){
				$p->sendMessage("§cNo Tienes Permisos Para Usar Esta Habilidad");
				return true;
			}
			...
			if(isset($this->skills[$name][0])){
				...
				if($hab1 == $hab){
					...
					$p->sendMessage("§c(-) Su habilidad: §f $hab §cha sido deshabilitada con éxito!");
					return true;
				}
				if($p->hasPermission("bridge.duple.hab")){
					if(isset($this->skills[$name][1])){
						...
						$p->sendMessage("§a(+) Tu habilidad: §f $hab §aha sido activada con éxito \n§8 \n§a (++) Tienes dos habilidades activadas: §f $hab1, $hab!");
						return true;
					}
				} else {
					$p->sendMessage("§c ¡Solo puedes elegir una habilidad!");
					return true;
				}
			} else {
				$this->skills[$name][0] = $hab;
				$p->sendMessage("§a(+) ¡Tu §f $hab §a se ha activado con éxito!");
				return true;
			}
		}
		return false;
	...
	public function initPlayers(){
		...
		if($count <= 0){
			...
			if($value){
				...
				//when the games end
				$p->addTitle("§l§7VICTORIA!", "§b§lTu Equipo Gano El Partido!");
				$this->segs = 10;
...
$api = Scoreboards::getInstance();
                $api->new($player, "ObjectiveName", "§l§eTheBridge");
		...
		unset($this->players[$name]);
		if($msg){
			$player->sendMessage("§bSaliste del THEBRIDGE");
		}
	}
